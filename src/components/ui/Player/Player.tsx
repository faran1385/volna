...
import {BoxVolumes} from "./BoxVolumes.tsx";
import {SET_CURRENT_INDEX} from "../../../app/Player/Player.ts";

export const Player = () => {
    const dispatch = useAppDispatch()
    const audio = useRef<HTMLAudioElement>(null);
    const player = useRef<null | HTMLDivElement>(null)
    const playerTrigger = useRef<null | HTMLButtonElement>(null)
    const {currentIndex, playlist} = useAppSelector((state) => state.player)
    const triggerPlayer = () => {
        (player.current as HTMLDivElement).classList.toggle("translate-y-full");
        (playerTrigger.current as HTMLButtonElement).classList.toggle("player__tigger-translate")
    }
    ...
    return <>
        <div ref={player}
            ...
            <div className=" object-cover w-full flex justify-center" style={{height: "100px"}}>
                <img alt={'music image'} loading="lazy" src={playlist[currentIndex].img}
                     className="rounded-xl player__img"/>
            </div>
            <div className="flex px-2.5 justify-center flex-wrap pt-2">
                <b className="player__title break-all	">{playlist[currentIndex].name}</b>
                <span className="text-fade">&nbsp; – &nbsp;</span>
                <span className="text-fade break-all">{playlist[currentIndex].singer}</span>
            </div>
            <div className="flex pt-2 justify-center items-center">
                <button className="mx-2" onClick={() => dispatch(SET_CURRENT_INDEX(-1))}>
            ...
            <div className="pt-2 flex justify-center">
                <div className="flex items-center w-4/5">
                    <ProcessInput audio={audio} href={playlist[currentIndex].href}/>
                </div>
            </div>
            <div className=" pt-3 flex justify-center">
                <div className="flex items-center justify-between w-4/5">
                    <BoxVolumes audio={audio}/>
                    <Tooltip title={"Playlist"}
                             slotProps={{popper: {modifiers: [{name: "offset", options: {offset: [0, -4]}}]}}} arrow>
                        <a href="#">
                            <svg className="player__control" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                                <path
                                    d="M15,13H9a1,1,0,0,0,0,2h6a1,1,0,0,0,0-2Zm0-4H9a1,1,0,0,0,0,2h6a1,1,0,0,0,0-2ZM12,2A10,10,0,1,0,22,12,10,10,0,0,0,12,2Zm0,18a8,8,0,1,1,8-8A8,8,0,0,1,12,20Z"></path>
                            </svg>
                        </a>
                    </Tooltip>
                </div>
        ...
        <div style={{zIndex: 11}} className="fixed w-full flex justify-center bottom-0 xl:hidden">
            <button ref={playerTrigger} onClick={triggerPlayer} className="flex player__trigger rounded-t-xl py-2 px-4">
