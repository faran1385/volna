...
export const HeaderLeft = ({ setToggleSearch, toggleSearch }: StateSearchType) => {
    return (
        <div className='Header-nav__left w-auto Header-nav__left--action   absolute  flex items-centerw'>
            ...
            <div className={`xl:w-auto xl:static Header-nav__left__inputbox--resize    transition-all duration-500 flex justify-between items-center xl:gap-2 fixed z-40  ${toggleSearch ? "top-0  " : "-top-36 "}`}>
                <SearchInput />
                <button className='xl:hidden pe-4 block' title='cluse' onClick={() => setToggleSearch(false)}>
