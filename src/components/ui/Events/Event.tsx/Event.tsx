...
export const Event: React.FC<EventProps> = (T) => {
    const { img, isAvailable, date, time, title, location } = T
    return <>

        <div className="relative event rounded-xl">
            <img className="rounded-xl w-full h-full event__image" src={img} />
            <div className="h-1/2  px-8 pt-8 top-0 absolute w-full">
                ...
                </button>) : (<div className="md:px-6 w-fit px-4 event__button event__button--unavailable flex items-center rounded-xl justify-center">
                    <span>Sold out</span>
                </div>)}
            </div>
            <div className="h-1/2  px-8 pb-8 bottom-0 absolute w-full">
