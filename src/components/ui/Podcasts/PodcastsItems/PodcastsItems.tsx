    ...
    return (
        <a onClick={clickHanler} className='podcasts__Items cursor-pointer w-full'>
            <div className={`podcasts__Items__header ${hasLive ? "podcasts__Items__header--active" : ""} relative overflow-hidden`}>
                <img className='podcasts__Items__header__img w-full' loading='lazy'  src={imgSrc} title={`podcast ${name}`}/>
                <span className='podcasts__Items__header__view absolute z-10 sm:bottom-7 sm:left-7 bottom-4 left-4'>{view > 999 ? view/1000+"K" : view } {hasLive ? "viewres" : "view"}</span>
