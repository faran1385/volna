...
export const NavigationButton: React.FC<NavigationButtonProps> = (T) => {
    ...
    return <>
        <button onClick={() => navigationHandler(whereTo, swiperRef)} className={`button h-1 me-2 rounded-xl w-3 button-${aditionalClass} button-${aditionalClass}--active ${isActive ?  `button--active` : ""}`}></button>
    </>
}