...
import {AboutCardPlan} from '../../components/ui/AboutCard/AboutCardPlan/AboutCardPlan'
...
const SubCard = [
    {
        ...
        icon: <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
            <path
                d="M19,7H18V6a3,3,0,0,0-3-3H5A3,3,0,0,0,2,6H2V18a3,3,0,0,0,3,3H19a3,3,0,0,0,3-3V10A3,3,0,0,0,19,7ZM5,5H15a1,1,0,0,1,1,1V7H5A1,1,0,0,1,5,5ZM20,15H19a1,1,0,0,1,0-2h1Zm0-4H19a3,3,0,0,0,0,6h1v1a1,1,0,0,1-1,1H5a1,1,0,0,1-1-1V8.83A3,3,0,0,0,5,9H19a1,1,0,0,1,1,1Z"></path>
        </svg>,
        backgroundColor: "",
        Color: "",
    },
...
export const PricingPlans = () => {
    ...
    return (
        <main className='contacts pages pages--mobile'>
            <HeadPages title='Pricing Plans' Items={[{text: "home", href: "/"}, {text: "pricing plans"}]}/>
            <p className='text-gary px-8 mb-8'>
                Many desktop publishing packages and <a href="#" className='about-link'>web page</a> editors now use
            ...
            <ul className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 px-8 pb-8 gap-6'>
                {
                    SubCard.map((e, index) => {
                        return (
                            <li key={`sub__about__card__${index}`}>
                                <AboutCardHeader index={0} text={e.text} title={e.title} icon={e.icon} Color={e.Color}
            ...
            <h3 className='about-title px-8 mb-5 pt-8'>
                Select Your plan
            </h3>
            <ul className='grid grid-cols-1 lg:grid-cols-3 px-8 mb-8 gap-6'>
                {
                    PlanCard.map((e, index) => {
                        return (
                            <li key={`plan__card__about__${index}`}>
                                <AboutCardPlan
                                    title={e.title}
                                    price={e.price}
                                    color={e.color}
